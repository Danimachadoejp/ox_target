...
local function startTargeting()
    ...
    while state.isActive() do
        ...
        if entityChanged and (not newOptions or hidden == totalOptions) then
            SendNuiMessage('{"event":"setTarget","data":null}')
        end
    ...
    state.setNuiFocus(false, false)
    SendNuiMessage('{"event":"setVisible","data":false}')
    SendNuiMessage('{"event":"setHasCursor","data":false}')
